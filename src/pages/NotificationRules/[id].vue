    ...
    >
      <template #[`item.createTime`]="{item}">
        <div v-if="item.createTime">
          <span style="white-space: nowrap;">{{ filters.date(item.createTime, 'local', store.getters.getConfig('dates').mediumDate) }}</span>
          <br>
      ...
      <template #[`item.data.reactivate`]="{item}">
        <div v-if="item.data.reactivate">
          <span style="white-space: nowrap;">{{ filters.date(item.data.reactivate, 'local', store.getters.getConfig('dates').mediumDate) }}</span>
          <br>
          ({{ filters.until(item.data.reactivate) }})
        </div>
      </template>
      <template 
        v-for="desc in ['service', 'receivers', 'days']"
        #[`item.data.${desc}`]="{item}"
      >
        {{ item.data[desc as 'service']?.join(', ') }}
      </template>
      <template 
        v-for="desc in ['active', 'useOnCall']"
        #[`item.data.${desc}`]="{item}"
      >
        <v-icon
      ...
      <template #[`item.data.triggers`]="{item}">
        <v-row
          v-for="(trigger, index) in item.data.triggers"
