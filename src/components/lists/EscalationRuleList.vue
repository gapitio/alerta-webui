<template>
  <v-row>
    <v-col cols="auto">
      <h1>
        {{ t('EscalationRule') }}
        <information-dialog :info="headers" :title="t('EscalationRuleInfo')">
          <template #tags>
            <tags-information />
          </template>
          <template #triggers>
      ...
      <v-row>
        ...
        <v-col cols="auto">
          <g-switch
            ...
            color="primary"
            @update:model-value="
              (value: boolean) => store.dispatch('escalationRules/setActiveFilter', {inactive: value})
            "
          />
        </v-col>
        <v-col cols="2" />
...
const headers = ref<
  {title: string; key: keyof EscalationRule | 'actions'; info?: string | string[]; sortable?: false}[]
>([
...
const escalationRules = computed(() =>
  store.state.escalationRules.items.map(b => {
    const period = {
      startTime: '',
      endTime: ''
    }
