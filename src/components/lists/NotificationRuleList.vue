      ...
      <v-row>
        ...
        >
          <g-switch 
            ...
            @update:model-value="(value: boolean) => store.dispatch('notificationRules/setActiveFilter', {active: value})"
          />
        </v-col>
        <v-col 
          cols="auto"
        >
          <g-switch 
            ...
            color="primary"
            @update:model-value="(value: boolean) => store.dispatch('notificationRules/setActiveFilter', {inactive: value})"
          />
        </v-col>
        <v-col cols="2" />
        <v-col 
    ...
    >
      ...
      <template #[`item.reactivate`]="{item}">
        <div v-if="item.reactivate">
          <span style="white-space: nowrap;">{{ filters.date(item.reactivate, 'local', store.getters.getConfig('dates').mediumDate) }}</span>
          <br>
...
const notificationRules = computed(() =>
  store.state.notificationRules.items
    .map(b => {
      const period = {
        startTime: '',
        endTime: ''
