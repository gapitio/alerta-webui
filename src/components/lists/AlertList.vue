<template>
  <v-data-table-server
    ...
    show-select
    fixed-header
    style="height: calc(99vh - calc(48px + 43px + 64px))"
...
const headersMap = computed(() => ({
  ...
  actions: { title: t('Actions'), key: 'actions', sortable: false },
}))
...
const customHeaders = computed(() => {
  const configHeaders = store.state.config.columns.map(
    (c) => headersMap.value[c as keyof typeof headersMap.value] ?? { text: c.charAt(0).toUpperCase() + c.slice(1), value: 'attributes.' + c }
  )
