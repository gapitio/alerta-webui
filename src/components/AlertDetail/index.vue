<template>
  <v-card flat>
    <v-card tile flat>
      ...
      <v-btn v-show="!isWatched(item?.customTags!)" variant="text" icon @click="watchAlert(item?.id!)">
        <v-icon icon="visibility" />
        <v-tooltip location="bottom" activator="parent" :text="t('Watch')" />
      </v-btn>

      <v-btn v-show="isWatched(item?.customTags!)" variant="text" icon @click="unwatchAlert(item?.id!)">
      ...
      <v-btn v-if="haveDeleteScope()" variant="text" icon @click="deleteAlert(item?.id!)">
        <v-icon icon="delete" />
        <v-tooltip location="bottom" activator="parent" :text="t('Delete')" />
      </v-btn>

      <v-btn :key="copyIconText" variant="text" icon @click="clipboardCopy(item)">
        <v-icon icon="content_copy" />
        <v-tooltip location="bottom" activator="parent" :text="copyIconText" />
      </v-btn>

      <v-card>
        <v-tabs v-model="active" grow slider-color="link-active">
          <v-tab value="detail">
            <v-icon size="x-large" icon="info" />
            &nbsp;
            {{ t('Details') }}
          </v-tab>
          <v-tab value="history">
            <v-icon icon="history" size="x-large" />
            &nbsp;
            {{ t('History') }}
          </v-tab>
          <v-tab value="data">
...
function haveDeleteScope() {
  ...
  if (store.state.config.delete_alert_scope_enforced)
    ...
    return (
      ...
      scopes.includes('admin:alerts') ||
      scopes.includes('write') ||
      scopes.includes('write:alerts') ||
      scopes.includes('delete:alerts')
    )
}
...
const takeAction = debounce(
  (id: string, action: string, text?: string) => {
    store.dispatch('alerts/takeAction', [id, action, text ?? '']).then(() => getAlert())
  },
  200,
...
const ackAlert = debounce(
  (id: string, text: string) => {
    store.dispatch('alerts/takeAction', [id, 'ack', text, ackTimeout.value!]).then(() => getAlert())
  },
  200,
  {leading: true, trailing: false}
)
...
const shelveAlert = debounce(
  (id: string, text?: string) => {
    store.dispatch('alerts/takeAction', [id, 'shelve', text ?? '', shelveTimeout.value!]).then(() => getAlert())
  },
  200,
  {leading: true, trailing: false}
)
...
const watchAlert = debounce(
  (id: string) => {
    store.dispatch('alerts/watchAlert', id).then(() => getAlert())
  },
  200,
  {leading: true, trailing: false}
)
...
const unwatchAlert = debounce(
  (id: string) => {
    store.dispatch('alerts/unwatchAlert', id).then(() => getAlert())
  },
  200,
  {leading: true, trailing: false}
)
...
const addNote = debounce(
  (id: string, text: string) => {
    store.dispatch('alerts/addNote', [id, text]).then(() => getNotes())
  },
  200,
  {leading: true, trailing: false}
)
