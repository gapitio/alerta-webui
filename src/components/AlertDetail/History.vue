    ...
    >
      ...
      <template #[`item.updateTime`]="{item}">
        <td class="text-no-wrap">
          <date-time :value="item.updateTime" format="mediumDate" />
        </td>
      </template>
      <template v-for="col in ['status', 'type']" #[`item.${col}`]="{item}" :key="col">
        <td class="text-no-wrap">
          <v-chip size="small" variant="flat" class="chip">
      ...
      <template #[`item.severity`]="{item}">
        <td class="text-no-wrap">
          <v-chip size="small" variant="flat" class="chip" :class="[`${item.severity}`]">
...
const history = computed(() => props.alert.history.map((h, index) => ({index: index, ...h})))
