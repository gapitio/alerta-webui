<template>
  <v-card max-width="350">
    <v-list>
      <v-list-item>
        ...
        <v-list-item-subtitle>
          <span>
            <span v-if="profile.preferred_username && !profile.preferred_username.includes('@')">@</span
        ...
        <template #append>
          ...
          >
            <template #activator="{props}">
              <v-icon :icon="provider[profile.provider].icon" v-bind="props" />
            </template>
          </v-tooltip>
          <v-tooltip v-else-if="profile.email_verified" location="top" :text="t('EmailVerified')">
            <template #activator="{props}">
              <v-icon icon="verified_user" v-bind="props" />
            </template>
          </v-tooltip>
          <v-tooltip v-else location="top" :text="t('EmailNotVerified')">
            <template #activator="{props}">
              <v-icon icon="person_cancel" v-bind="props" />
            </template>
          </v-tooltip>
        </template>
      </v-list-item>
    ...
    <v-list>
      <profile-info v-if="customerViews" :items="customers" :title="t('Customers')" />

      <profile-info v-if="profile.orgs" :items="profile.orgs" :title="t('Organizations')" />

      <profile-info v-if="profile.groups" :items="profile.groups" :title="t('Groups')" />

      <profile-info v-if="profile.roles" :items="profile.roles" :title="t('Roles')" />

      <profile-info :items="scopes" :title="t('Scopes')" />
    </v-list>
...
function logout() {
  ...
  store.dispatch('auth/logout').then(response => {
    if (response.data.logoutUrl) {
      ...
      const redirectUrl =
        (provider == 'keycloak' ? 'redirect_uri=' : 'post_logout_redirect_url=') +
