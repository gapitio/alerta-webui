      ...
      <v-card-text>
        ...
        >
          ...
          <v-col cols="10">
            <v-row no-gutters>
              <v-col cols="12">
                {{ t('TextFromTrigger') }}: {{ e.triggerText }}
              </v-col>
              <v-col cols="12">
                {{ t('TextFromNotificationRule') }}: {{ e.notificationRuleText }}
              </v-col>
              <v-col cols="12">
                {{ t('Result') }}: {{ e.result }}
              </v-col>
...
const examples = [
  {triggerText: '', notificationRuleText: '', result: 'Production: critical alert for 172.10.14.63, server - many0fk4 is server failure'},
  {triggerText: '{default}', notificationRuleText: 'This is the default text', result: 'This is the default text',},
  {triggerText: 'Got alarm {default} with {severity} severity', notificationRuleText: 'from {resource}', result: 'Got alarm from many0fk4 with critical severity'},
  {triggerText: '{tags} {resource} got {severity} {event} alarm from {attributes.ip}', notificationRuleText: 'from {resource} with {event}', result: 'test, a, b many0fk4 got Critical server failure alarm from 127.0.0.1'},
]
