    ...
    <v-form ref="form">
      <v-card class="dialog-card">
        ...
        <v-card-text style="overflow-x: hidden">
          <v-row>
            <v-col cols="12">
              <g-combobox v-model="editedItem.message" show-header :label="t('Message')" clearable multiple />
            </v-col>
            <v-col cols="12">
              <g-combobox v-model="editedItem.receiver" show-header :label="t('Receiver')" clearable multiple />
            </v-col>
            <v-col cols="12">
              <g-combobox v-model="editedItem.channel" show-header :label="t('Channel')" clearable multiple />
            </v-col>
            <v-col cols="12">
              <g-combobox v-model="editedItem.rule" show-header :label="t('Rule')" clearable multiple />
            </v-col>
            <v-col cols="12">
              <g-combobox v-model="editedItem.alert" show-header :label="t('Alert')" clearable multiple />
            </v-col>
            <v-col cols="12">
              <g-combobox v-model="editedItem.error" show-header :label="t('Error')" clearable multiple />
            </v-col>
            <v-col cols="12">
              <g-select v-model="filterDateRange" show-header :items="dateRanges" :label="t('SelectRange')" />
            </v-col>
            <v-col v-if="showSelectDate" cols="6">
              <date-edit v-model="period.startDate" show-header :label="t('StartDate')" />
            </v-col>
            <v-col v-if="showSelectDate" cols="6">
              <g-combobox v-model="period.startTime" show-header :items="times" :label="t('StartTime')" />
            </v-col>
            <v-col v-if="showSelectDate" cols="6">
              <date-edit v-model="period.endDate" show-header :label="t('EndDate')" />
            </v-col>
            <v-col v-if="showSelectDate" cols="6">
              <g-combobox v-model="period.endTime" show-header :items="times" :label="t('EndTime')" />
            </v-col>
          </v-row>
        </v-card-text>
...
function apply() {
  ...
  filter.value = {
    ...
    dateRange: {
      ...
      from: !editedItem.value.dateRange.select
        ? editedItem.value.dateRange.from
        : toEpoch(period.value.startDate!, period.value.startTime!),
      to: !editedItem.value.dateRange.select
        ? editedItem.value.dateRange.to
        : toEpoch(period.value.endDate!, period.value.endTime!)
    }
  }
}
</script>
