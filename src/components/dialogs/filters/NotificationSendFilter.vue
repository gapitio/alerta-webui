      ...
      <v-card class="dialog-card">
        ...
        <v-card-text style="overflow-x: hidden">
          <v-row>
            ...
            <v-col cols="12">
              <g-select v-model="editedItem.type" show-header :items="types" :label="t('Type')" clearable />
            </v-col>
          </v-row>
        </v-card-text>
...
import type {Filter} from '@/plugins/store/types/notificationSends-types'
...
const filter = computed({
  get: () => store.state.notificationSends.filter,
  set: val => store.dispatch('notificationSends/setFilter', val)
})
...
const getNotificationSends = () => store.dispatch('notificationSends/getNotificationSends')
