        ...
        <v-card-text style="overflow-x: hidden">
          <v-row>
            ...
            <v-col cols="6" class="pb-0">
              <date-edit v-model="editedItem.startDate" show-header :label="t('StartDate')" />
            </v-col>
            <v-col cols="6" class="pb-0">
              <date-edit v-model="editedItem.endDate" show-header :label="t('EndDate')" />
            </v-col>

            <v-col cols="6" class="pb-0">
              <g-combobox v-model="editedItem.period.start" show-header :items="times" :label="t('StartTime')" />
            </v-col>
            <v-col cols="6" class="pb-0">
              <g-combobox
            ...
            <v-col cols="12" class="pb-0">
              <g-select v-model="editedItem.repeatDays" show-header multiple :items="days" :label="t('Days')" />
            </v-col>
            ...
            <v-col cols="12" class="pb-0">
              ...
              >
                <template #prepend-item>
                  <v-list-item title="OddWeeks" @click="selectOdd" />
                  <v-list-item title="EvenWeeks" @click="selectEven" />
                </template>
              </g-combobox>
            </v-col>
...
const selectOdd = () =>
  (editedItem.value.repeatWeeks = Array.from(
    new Set([...editedItem.value.repeatWeeks, ...weeks.filter((value, index) => index % 2 == 0)])
  ))
const selectEven = () =>
  (editedItem.value.repeatWeeks = Array.from(
    new Set([...editedItem.value.repeatWeeks, ...weeks.filter((value, index) => index % 2 == 1)])
  ))
