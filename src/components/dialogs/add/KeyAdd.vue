      ...
      <v-card class="dialog-card">
        ...
        <v-card-text style="overflow-x: hidden">
          <v-row>
            ...
            <v-col cols="12" class="pb-0">
              <date-edit v-model="editedExpireDate" :label="t('Expires')" show-header />
            </v-col>

            <v-col cols="12" class="pb-0">
              <g-text-field v-model="editedItem.text" show-header :label="t('Comment')" />
            </v-col>
          </v-row>
        </v-card-text>
...
async function save() {
  if (editedItem.value.id) {
    ...
    await store.dispatch('keys/createKey', Object.assign(editedItem.value, {}))
  }
  close(true)
}
