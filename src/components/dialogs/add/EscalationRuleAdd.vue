...
const editedItem: Ref<EscalationRule & {
  timeObj: {
    time: string | null,
...
const valueStart: Ref<EscalationRule & {
  timeObj: {
    time: string | null,
    interval: string | null
  },
      ...
      const obj = {
        ...
        period: {
          startTime: props.item.startTime
            ? filters.hhmmUtcToLocal(props.item.startTime)
            : '',
          endTime: props.item.endTime
            ? filters.hhmmUtcToLocal(props.item.endTime)
            : ''
        }
      }
...
async function save() {
  ...
  if (editedItem.value.id) {
      ...
      editedItem.value.id,
      {
        ...
        time: editedItem.value.timeObj.time ? `${editedItem.value.timeObj.time} ${editedItem.value.timeObj.interval}` : null,
        startTime: sTimeStr,
        endTime: eTimeStr,
