  ...
  <v-dialog v-model="dialog" max-width="540px">
    <v-form ref="form">
      <v-card class="dialog-card">
        <v-card-title>
          <v-col cols="12">
            <span class="headline"> {{ activate ? t('Activate') : t('Deactivate') }} {{ types[type].title }} </span>
          </v-col>
        </v-card-title>
        ...
        <v-card-text>
          <v-row v-if="!hideTime && !activate">
            <v-col cols="6">
              <date-edit v-model="editedDate" show-header :label="t('ReactivateDate')" />
            </v-col>
          ...
          <v-row>
            <v-col cols="12">
              ...
              <ul style="padding-left: 20px">
                <li v-for="object in selected" :key="object.id">
        ...
        <v-card-actions class="dialog-card-actions">
          <v-col cols="6">
            <v-btn variant="outlined" class="no-cap-btn btn" width="247" @click="emit('close')">
              {{ t('Cancel') }}
            </v-btn>
          </v-col>
          <v-col cols="6">
            <v-btn color="primary-600" variant="flat" class="no-cap-btn" width="247" @click="changeBulkState()">
              {{ t('Save') }}
            </v-btn>
          </v-col>
        </v-card-actions>
...
const types: {nr: Type; er: Type} = {
  nr: {
    value: 'notificationRules/updateNotificationRule',
    title: t('NotificationRules'),
    store: store.state.notificationRules
  },
...
const selected = computed(() =>
  types[compProps.type].store.items.filter(({id}: {id: string}) => types[compProps.type].store.selected.includes(id))
)
...
function changeBulkState() {
  selected.value.map(({id}: {id: string; [key: string]: any}) => {
    store.dispatch(types[compProps.type].value, [
      id,
      {
        active: compProps.activate,
        reactivate:
          editedDate.value && item.value.time && !compProps.activate
            ? toISODate(editedDate.value, item.value.time)
            : null
      }
    ])
  })
