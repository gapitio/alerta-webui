...
const filename = ref('Alerts')
const length = ref(1000)
type Key = keyof Alert
...
const customKeys = computed(() =>
  store.state.config.columns.map(a => (allKeys.value.includes(a) ? a : `attributes.${a}`))
)
const keys = ref<(Key | string)[]>(JSON.parse(JSON.stringify(customKeys.value)))

let itemsPerPage = 20
const isLoading = computed(() => store.state.notificationHistory.isLoading)

watch(dialog, val => {
  if (val) itemsPerPage = store.state.alerts.pagination.itemsPerPage
  else store.dispatch('alerts/setPagination', {itemsPerPage, page: 1})
  filename.value = 'Alerts'
  length.value = 1000
  keys.value = JSON.parse(JSON.stringify(customKeys.value))
})

const getAlerts = () => store.dispatch('alerts/getAlerts')
const selectAll = () => (keys.value = allKeys.value)
const selectCustom = () => (keys.value = customKeys.value)
...
async function apply() {
  await store.dispatch('alerts/setPagination', {itemsPerPage: length.value, page: 1})
  const d = await store.dispatch('alerts/getAlerts')
  store.dispatch('alerts/setPagination', {itemsPerPage, page: 1})
  toCsv(d)
}
</script>
