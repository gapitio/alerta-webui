<template>
  <v-app>
    <div vi-if="!isKiosk">
      ...
      <v-toolbar flat class="toolbar bg-surface-tertiary">
        <v-spacer />

        <v-btn v-show="isLoggedIn || !isAuthRequired || isAllowReadonly" icon @click="toggleFullScreen">
          <v-icon :icon="isFullscreen ? 'fullscreen_exit' : 'fullscreen'" />
          <v-tooltip location="bottom" activator="parent" :text="t('FullScreen')" />
        </v-btn>

        <v-btn v-show="isLoggedIn || !isAuthRequired || isAllowReadonly" icon @click="refresh">
          <v-icon icon="refresh" />
          <v-tooltip location="bottom" activator="parent" :text="t('Refresh')" />
        </v-btn>
        <v-menu v-if="isLoggedIn" v-model="menu" :close-on-content-click="false" :nudge-width="200" offset-x>
          <template #activator="{props}">
            <v-btn v-bind="props" icon>
              <v-avatar size="32px">
                <img v-if="avatar && !error" :src="avatar" @error="error = true" />
                <v-icon v-else icon="account_circle" />
              </v-avatar>
            </v-btn>
          </template>
        ...
        <span class="hidden-xs-only">
          ...
          <v-btn v-show="!isLoggedIn" rounded="xl" variant="elevated" color="primary" to="/login" class="ma-2">
            {{ t('LogIn') }}
          </v-btn>
        </span>
      </v-toolbar>
...
const isDark = computed(() => {
  return store.state.prefs.isDark
})
